...
for i, theta in enumerate(thetas):
    vv_points[i] = generate_point(theta, 100) #offset normally 0
    blanket_points[i] = generate_point(theta, blanket_thickness)

# Save points to text file
# np.savetxt(str(sys.argv[1])+"_vv_split.txt", vv_points)
# np.savetxt(str(sys.argv[1])+"_blanket_split.txt", blanket_points)
